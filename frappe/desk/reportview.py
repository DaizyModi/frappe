...
def delete_report(name):
	...
	if frappe.db.exists('Report', name):
		d = frappe.db.get('Report', name)
		if d.is_standard == "Yes":
			frappe.throw(_("Standard Reports can't be deleted"))

		if d.owner != frappe.session.user:
			frappe.throw(_("Report owner can delete the report"))

		if d.report_type == "Report Builder":
			frappe.delete_doc("Report", d.name, ignore_permissions=True)
			frappe.db.commit()
			frappe.msgprint(_("{0} is Deleted").format(d.name), alert=True)
		else:
			frappe.throw(_("Report Builder Type reports can be deleted only"))
	else:
		frappe.msgprint(_("Report does not exists"))
